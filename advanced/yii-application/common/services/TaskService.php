...
use common\services\events\UserCompleteTaskEvent;
...
class TaskService extends Component
{
    ...
    const EVENT_USER_COMPLETE_TASK = 'event_user_complete_task';
    ...
    function userTakeTask(Project $project, $user, Task $task)
{
    ...
    function userCompleteTask(Project $project, $user, Task $task)
    {

        $event = new UserCompleteTaskEvent();
        $event->project = $project;
        $event->user = $user;
        $event->task = $task;
        $event->tester = User::find()->select('email')
            ->innerJoin('project_user pu', 'user.id = pu.user_id')
            ->innerJoin('project', 'project.id = pu.project_id')
            ->andWhere(['pu.role' => 'tester'])
            ->andWhere(['pu.project_id' => $project->id])
            ->one();// не красиво получилось. исправлю. в данный момент так функционирует.
        $this->trigger(self::EVENT_USER_COMPLETE_TASK, $event);
    }
    /**
     * @param Project $project
     * @param User $user
